 ...
 *
 */
public class IterativeGraphTraverserModule extends GraphModule
{
	/**
	...
	private static class StackEntry
	{
		...
		public StackEntry(Edge edge, long order)
		{
			this.edge = edge;
			this.order = order;
		}
	...
	class Traverser implements Runnable
	{
				...
				{//TOP_DOWN_DEPTH_FIRST traversal
					for (Node startNode: startNodes)
					{
						...
						if (traverseHandler.checkConstraint(GRAPH_TRAVERSE_TYPE.TOP_DOWN_DEPTH_FIRST, traverseId, null, startNode, 0l))
						{
							...
							this.todoList.add(new StackEntry(null, 0));
							while(!todoList.isEmpty())
							{
								iterationStep(GRAPH_TRAVERSE_TYPE.TOP_DOWN_DEPTH_FIRST);
								// just repeat iteration step.
							}
						}
					}
				}//TOP_DOWN_DEPTH_FIRST traversal
        ...
        {
          throw new GraphTraverserException("Unsupported traversion type " + traverseType);
        }
			} catch (Exception e) 
			{
...
//						}
		...
		private final ArrayList<StackEntry> todoList = new ArrayList<StackEntry>();
		
		/**
		 */
		private void iterationStep(GRAPH_TRAVERSE_TYPE type)
		{
			...
			StackEntry first = todoList.remove(0);
			Node currNode;
			if(first.getEdge() == null)
			{
				currNode = currentNodePath.get(currentNodePath.size()-1);
			}
			else
			{
				currNode = first.getEdge().getTarget();
				currentNodePath.add(currNode);
			}
			...
			traverseHandler.nodeReached(type, traverseId, 
					currNode, first.getEdge(), parent, first.getOrder());
			{//walk through all childs of this node
				...
				{//in case of node has childs
					...
					for(Edge childEdge: childEdges)
					{
						...
						if (traverseHandler.checkConstraint(type, traverseId, childEdge, 
								childNode, first.getOrder()))
						{
							if (	(this.isCycleSafe)&&
							...
							todoList.add(i, new StackEntry(childEdge, i));
			...
			traverseHandler.nodeLeft(type, 
					traverseId, currNode, first.getEdge(), parent, first.getOrder());
